# Moltbot HA Add-on - Troubleshooting Guide

Solutions to common problems and issues with the Moltbot Home Assistant Add-on.
...
```bash
# Check add-on status
ha addons info local_moltbot

# View recent logs
ha addons logs local_moltbot

# Optional: SSH into container (only if `ssh_authorized_keys` is set)
...
**Still failing?** Report issue with logs: [GitHub Issues](https://github.com/Al3xand3r1987/moltbot-ha/issues)
...
```bash
ha addons logs local_moltbot --follow
```
...
4. **Reset to defaults:**
   ...
   - Try starting with minimal config:
     ```yaml
     update_mode: stable
     # Optional: enable SSH for debugging only (key-based)
     ssh_port: 2222
...
3. **Check for corrupt cache:**
   ```bash
   ssh -p 2222 root@YOUR-HA-IP
   rm -rf /config/moltbot/cache/*
   # Restart add-on
   ```

4. **Check smoke test logs:**
   ```bash
   ha addons logs local_moltbot | grep "smoke test"
   ```
...
2. **Check add-on is running:**
   ```bash
   ha addons info local_moltbot | grep state
   ```
...
6. **Update SSH key:**
   - Replace/add `ssh_authorized_keys` in the add-on configuration
   - Restart add-on
...
1. **Check update mode:**
   ```bash
   ha addons info local_moltbot | grep update_mode
   ```
...
5. **Clear source repository:**
   ```bash
   ssh -p 2222 root@YOUR-HA-IP
   rm -rf /config/moltbot/source/moltbot-src
...
1. **Check HA notifications:**
   ...
   - Look for "Moltbot" notifications

2. **Check logs:**
   ```bash
   ha addons logs local_moltbot | grep "notification"
   ```
...
4. **Manual check:**
   ```bash
   ssh -p 2222 root@YOUR-HA-IP
   cd /config/moltbot/source/moltbot-src
   git fetch --tags
...
3. **Check rollback logs:**
   ```bash
   ha addons logs local_moltbot | grep -i "rollback\|crash"
   ```
...
4. **Force rebuild:**
   ...
   ssh -p 2222 root@YOUR-HA-IP
   rm -rf /config/moltbot/cache/*
   rm /config/moltbot/.meta/current_version
   ```
...
2. **Check restore detection:**
   ```bash
   ha addons logs local_moltbot | grep "snapshot restore"
   ```
...
1. **Check data directory:**
   ```bash
   ls -la /config/moltbot/data/
   cat /config/moltbot/data/moltbot.json
   ```

2. **Check migration:**
   ```bash
   ha addons logs local_moltbot | grep "migration"
   ```

3. **Manual restore from backup:**
   - If you have manual backups of `moltbot.json`
   - Copy to `/config/moltbot/data/moltbot.json`
   - Restart add-on

4. **Prevention:**
   - Create snapshots regularly
   - Keep manual backup of `moltbot.json`
...
2. **Check snapshot version:**
   ```bash
   cat /config/moltbot/.meta/snapshot_version
   ```
...
2. **Check where key is configured:**
   - Moltbot config: `anthropic_api_key` in `/config/moltbot/data/moltbot.json` (or via the Web UI)
   - Prefer Moltbot config for security
...
2. **Check Ingress configuration:**
   ```bash
   ha addons info local_moltbot | grep ingress
   ```
...
4. **Check Gateway logs:**
   ```bash
   ha addons logs local_moltbot | grep "started on port"
   ```
...
4. **Check bot logs:**
   ```bash
   ha addons logs local_moltbot | grep -i telegram
   ```
...
3. **Manual cleanup:**
   ...
   cd /config/moltbot/cache
   ls -lt  # List by date
   rm -rf <old-version>/  # Remove old ones

   # Clean workspace if large
   du -sh /config/moltbot/data/workspace/*
   ```
...
# Version-related
ha addons logs local_moltbot | grep -i version

# Error messages
ha addons logs local_moltbot | grep -i error

# Snapshot/restore
ha addons logs local_moltbot | grep -i snapshot
...
```bash
ha addons logs local_moltbot > moltbot-logs.txt
```
...
# Backup data
cp -r /config/moltbot/data /config/moltbot-backup

# Remove cache and source
rm -rf /config/moltbot/cache
rm -rf /config/moltbot/source
rm -rf /config/moltbot/.meta
...
Check `/config/moltbot/cache/` for available versions.
...
1. **Gather information:**
   ```bash
   ha addons info local_moltbot > addon-info.txt
   ha addons logs local_moltbot > addon-logs.txt
...
3. **Search existing issues:**
   - [GitHub Issues](https://github.com/Al3xand3r1987/moltbot-ha/issues)

### Request Support:

- **GitHub Issues:** [Create Issue](https://github.com/Al3xand3r1987/moltbot-ha/issues/new)
- **Include:**
