...
- **HTTPS Warning**: Visual indicator when accessing Setup UI without HTTPS
  - Shows "HTTPS recommended" pill when `window.isSecureContext === false`
...
- **Direct API Key Configuration**: API keys can now be set directly in Home Assistant Add-on configuration
  ...
  - **Auto-Apply**: API keys from add-on config are automatically applied to Moltbot config on startup
...
- Reads API keys from `/data/options.json` and writes them to Moltbot config
...
- **Memory Plugin Validation Error**: Fixed crash loop caused by `memory-core` plugin not found
  ...
  - **Root cause**: Minimal fallback config didn't specify plugin settings, so Moltbot merged its defaults which reference `memory-core` - a plugin not available in v2026.1.23
...
- This prevents Moltbot from trying to load the default memory plugin
- Memory functionality can be re-enabled later via the Moltbot config UI
...
### Fixed
- **Setup Failure Tolerance**: Added fallback when moltbot setup fails
...
- Allows gateway to run without full Moltbot features
...
### Fixed
- **CRITICAL: Missing TypeScript sources in Cache**: Fixed runtime build failures in moltbot setup
  - **Root cause**: Cache missing `tsconfig.json` and `src/` directory
  - Moltbot tries to rebuild itself on first run, but TypeScript sources weren't cached
...
- Fixes startup errors for `moltbot setup` and `moltbot gateway` commands
...
- armv7 support maintained for core Moltbot functionality
...
- **Sidebar Panel**: Moltbot icon appears in Home Assistant sidebar
...
- `panel_title`: Title for sidebar panel (default: `Moltbot`)
...
### Added
- **Optional Ingress Setup UI (no SSH)**: `easy_setup_ui` adds `/__setup/` to run the Moltbot wizard (OpenAI Codex/ChatGPT OAuth) and store API keys (OpenAI/Anthropic) via `.env`.
...
2. **Update Add-on**:
   - Add-ons → Moltbot Gateway → Check for Updates → Update
...
- **API Keys**: Should be stored in Moltbot config, not add-on config
...
- **Repository**: [github.com/Al3xand3r1987/moltbot-ha](https://github.com/Al3xand3r1987/moltbot-ha)
- **Original Fork**: [github.com/ngutman/moltbot-ha-addon](https://github.com/ngutman/moltbot-ha-addon)
- **Issues**: [github.com/Al3xand3r1987/moltbot-ha/issues](https://github.com/Al3xand3r1987/moltbot-ha/issues)
- **Moltbot**: [github.com/moltbot/moltbot](https://github.com/moltbot/moltbot)

---

**Thank you for using Moltbot HA Add-on! 🎉**
