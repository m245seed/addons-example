...
/**
 * Moltbot HA Add-on Setup Proxy
 *
 * - Listens on 127.0.0.1:8099 (Ingress entry)
 * - Reverse-proxies HTTP + WebSocket to the running Moltbot Gateway (default 127.0.0.1:18789)
...
const MOLTBOT_ACTIVE_DIR = process.env.MOLTBOT_ACTIVE_DIR || '/config/moltbot/source/active';
const MOLTBOT_STATE_DIR = process.env.MOLTBOT_STATE_DIR || '/config/moltbot/data/state';
const MOLTBOT_CONFIG_PATH = process.env.MOLTBOT_CONFIG_PATH || '/config/moltbot/data/moltbot.json';

const ENV_PATH = path.posix.join(MOLTBOT_STATE_DIR, '.env');
...
function configHasApiKey() {
  if (!exists(MOLTBOT_CONFIG_PATH)) return false;
  try {
...
function shouldServeSetupForRequest(reqUrl) {
  ...
  if (!exists(MOLTBOT_CONFIG_PATH)) return true;
  // Show setup if config is minimal (no API key)
  return !configHasApiKey();
}

function pnpmCwd() {
  // The add-on ensures /config/moltbot/source/active points to a built version.
  // If it's not there yet, we still try; errors are surfaced to the UI.
  return MOLTBOT_ACTIVE_DIR;
}
...
async function gatewayCall(method, paramsObj, { token, timeoutMs } = {}) {
  ...
  const args = ['moltbot', 'gateway', 'call', method, '--url', gatewayWsUrl(), '--params', paramsRaw, '--json'];
...
function serializeDotEnv(map) {
  ...
  const lines = [
    '# Generated by Moltbot HA Add-on Setup UI',
...
<html lang="de">
  <head>
    ...
    <title>Moltbot Setup (Home Assistant)</title>
    <style>
  ...
  <body>
    <div class="wrap">
      <div class="hero">
        <div>
          <h1>Moltbot Setup (Home Assistant)</h1>
          <p>Simple setup without SSH: configure ChatGPT/Codex OAuth or set API keys.</p>
        </div>
        <div class="pill" id="statusPill">Loading status…</div>
        <span id="httpsWarning" class="pill warn" style="display:none; margin-left: 8px;">HTTPS recommended</span>
      </div>

      <div class="card">
        <div class="row">
          <div class="col">
            <h2 style="margin:0 0 6px; font-size:16px;">1) Wizard (recommended)</h2>
            <p class="muted" style="margin:0 0 10px;">Starts the official Moltbot wizard via Gateway RPC. There you can choose “OpenAI Codex (ChatGPT OAuth)”.</p>
            <div class="row" style="gap:8px;">
              <button class="primary" id="btnWizardStart">Start wizard</button>
              <button id="btnWizardStatus">Status</button>
              <button id="btnWizardNext">Next</button>
              <button id="btnWizardCancel">Cancel</button>
            </div>
            <label for="gatewayToken">Optional: Gateway token (if you enabled gateway auth)</label>
            <input id="gatewayToken" type="password" placeholder="leave empty if not needed" />
            <p class="small muted">Note: The token is only used for RPC calls and is not persisted in the browser.</p>
            <label for="wizardNextParams">Wizard next (JSON)</label>
            <textarea id="wizardNextParams" placeholder='Example: {"choice":"openai-codex"}'></textarea>
            <p class="small muted">If you’re not sure what to put here: click “Status” first and review the current step.</p>
          </div>
          <div class="col">
            <h2 style="margin:0 0 6px; font-size:16px;">2) API keys (fallback)</h2>
            <p class="muted" style="margin:0 0 10px;">Writes keys to <code>/config/moltbot/data/state/.env</code>. You can change this later at any time.</p>
            ...
            <p class="small muted">Keys are never shown in plaintext. Logs contain no secrets.</p>
          </div>
        </div>
      </div>

      <div class="card">
        <h2 style="margin:0 0 6px; font-size:16px;">Output</h2>
        <p class="muted" style="margin:0 0 10px;">Here you’ll see responses from RPC/CLI calls (truncated, without secrets).</p>
        <pre id="out">(no output yet)</pre>
        <div id="wizardHelper" style="margin-top:12px;"></div>
      </div>

      <div class="card">
        <h2 style="margin:0 0 6px; font-size:16px;">Continue to the normal UI</h2>
        <p class="muted" style="margin:0 0 10px;">Once everything is set up, you can switch to the normal Moltbot Control UI.</p>
        <a href="/" id="linkControlUi">Open Control UI</a>
      </div>

      <p class="small muted" style="margin-top:14px;">Path: <code>/__setup</code>. (Only active when <code>easy_setup_ui</code> is enabled in the add-on.)</p>
    </div>
    ...
    <script>
      ...
      function getActionsFromEnvelope(env) {
        ...
        for (const a of flat) {
          ...
          const label = a.label || a.title || a.name || a.text || a.id || 'Next';
      ...
      function renderWizardHelper(env) {
        ...
        h.innerHTML = '<strong>Wizard quick buttons</strong><div class="small muted">If the wizard provides actions, you can click them here (otherwise use “Wizard next (JSON)”).</div>';
        wrap.appendChild(h);
      ...
      document.getElementById('btnWizardNext').addEventListener('click', async () => {
        ...
        try {
          ...
          let params = null;
          try { params = JSON.parse(raw); } catch { throw new Error('Invalid JSON in “Wizard next (JSON)”'); }
          const r = await api('/wizard/next', {
            method: 'POST',
            body: JSON.stringify({ token: gatewayToken.value || null, params })
          });
      ...
      document.getElementById('btnCheckModels').addEventListener('click', async () => {
        setOut('Model/Auth Loading status…');
        try {
...
async function handleSetupApi(req, res) {
  ...
  if (req.method === 'GET' && pathOnly === '/env') {
    return sendJson(res, 200, {
      ...
      activeDir: MOLTBOT_ACTIVE_DIR,
      activeDirExists: exists(MOLTBOT_ACTIVE_DIR),
      stateDir: MOLTBOT_STATE_DIR,
      configPath: MOLTBOT_CONFIG_PATH,
      configExists: exists(MOLTBOT_CONFIG_PATH),
      envPath: ENV_PATH,
    });
  }

  if (req.method === 'GET' && pathOnly === '/models/status') {
    const { code, stdout, stderr } = await runPnpm(['moltbot', 'models', 'status', '--json'], { timeoutMs: 60000 });
    const parsed = safeJsonParse(stdout.trim());
...
function proxyHttp(req, res) {
  ...
  upstream.on('error', () => {
    sendText(
      res,
      502,
      'Gateway is not reachable yet. Please wait a few minutes and try again.\n'
    );
  });
