...
# ============================================================================
# PHASE 2: New directory structure (v1.0.0)
# ============================================================================
# Target layout:
# /config/moltbot/
...
# New directory definitions (v1.0.0)
DATA_DIR="${BASE_DIR}/data"
CACHE_DIR="${BASE_DIR}/cache"
META_DIR="${BASE_DIR}/.meta"
SOURCE_DIR="${BASE_DIR}/source"
REPO_DIR="${SOURCE_DIR}/moltbot-src"
WORKSPACE_DIR="${DATA_DIR}/workspace"
STATE_DIR="${DATA_DIR}/state"
CONFIG_PATH="${DATA_DIR}/moltbot.json"
SSH_AUTH_DIR="${BASE_DIR}/.ssh"

# Legacy paths for migration
LEGACY_STATE_DIR="${BASE_DIR}/.moltbot"
LEGACY_REPO_DIR="${BASE_DIR}/moltbot-src"
...
# ============================================================================
# Automatic migration from old structure (ONE TIME)
# ============================================================================
migrate_legacy_structure() {
  local migrated=false

  # 1. State-Daten migrieren (.moltbot -> data/state)
  if [ -d "${LEGACY_STATE_DIR}" ] && [ ! -f "${CONFIG_PATH}" ]; then
    log "migrating old .moltbot structure to new layout"

    # Copy state data (don’t move, for safety)
    cp -a "${LEGACY_STATE_DIR}/." "${STATE_DIR}/" 2>/dev/null || true

    # Haupt-Config verschieben
    if [ -f "${STATE_DIR}/moltbot.json" ]; then
      mv "${STATE_DIR}/moltbot.json" "${CONFIG_PATH}"
...
# ============================================================================
# Symlinks for /root (for tools that ignore $HOME)
# ============================================================================
...
# Legacy: /root/.moltbot migration
if [ -d /root/.moltbot ] && [ ! -f "${CONFIG_PATH}" ]; then
...
# Legacy: /root/moltbot-src migration
...
export MOLTBOT_CACHE_DIR="${CACHE_DIR}"
export MOLTBOT_META_DIR="${META_DIR}"
...
# Update check: determine whether a new version is available
check_for_updates() {
...
download_and_build_version() {
  ...
  local temp_dir="/tmp/moltbot-build-${version}-$$"
  rm -rf "${temp_dir}"
  mkdir -p "${temp_dir}"

  # Source-Code in temp kopieren
  cp -r "${REPO_DIR}" "${temp_dir}/moltbot-src" || {
    log "failed to copy source"
    rm -rf "${temp_dir}"
    return 1
  }

  cd "${temp_dir}/moltbot-src" || {
    rm -rf "${temp_dir}"
    return 1
  }
  ...
  # TypeScript source files (if a runtime build is needed)
...
# Clean up old versions (cache management)
cleanup_old_versions() {
  local max_versions="${1:-2}"
  local current_version="$2"

  # All versions sorted by modification time
  local versions=($(ls -t "${CACHE_DIR}" 2>/dev/null))
  local count=0

  for version in "${versions[@]}"; do
    ((count++))
    # Keep the newest max_versions, delete the rest (except current)
...
# Logic: If last_run_version is missing or doesn’t match snapshot_version,
...
# Store snapshot version for next boot
...
send_ha_notification() {
  local title="$1"
  local message="$2"
  local notification_id="${3:-moltbot_notification}"
...
# Auth for private repos (without leaking tokens in logs/remote URL)
...
# ============================================================================
if [ "${IS_SNAPSHOT_RESTORE}" != "true" ]; then
  # Ensure source repo is present (keep it up to date for updates)
    ...
    if [ "${UPDATE_MODE}" = "notify" ]; then
      ...
      if download_and_build_version "${NEW_VERSION}"; then
        if activate_version "${NEW_VERSION}"; then
          ...
          # Send HA notification about a successful update
          send_ha_notification \
            "Moltbot Updated" \
          ...
          # Send HA notification about a failed activation
          send_ha_notification \
            "Moltbot Update Failed" \
            "Failed to activate version ${NEW_VERSION}, staying on ${CURRENT_VER}" \
            "moltbot_update_failed"
        fi
      else
        log "update to ${NEW_VERSION} failed, staying on ${CURRENT_VER}"

        # Send HA notification about a failed build
        send_ha_notification \
          "Moltbot Update Failed" \
  ...
  # Store snapshot marker for next boot
  save_snapshot_marker "${CURRENT_VER}"
...
# Moltbot Setup (nur bei erster Installation)
if [ ! -f "${CONFIG_PATH}" ]; then
  log "running moltbot setup"
  # Setup can fail due to plugin issues — not critical for the gateway
  pnpm moltbot setup --workspace "${WORKSPACE_DIR}" || {
    log "setup failed (non-critical), creating minimal config"
    # Create minimal config if setup fails
...
apply_addon_api_keys() {
  ...
  node -e "
    ...
    const path = process.env.MOLTBOT_CONFIG_PATH;
...
# Apply API keys from add-on configuration to Moltbot config
if [ -f "${CONFIG_PATH}" ]; then
...
start_setup_proxy() {
  ...
  export MOLTBOT_ACTIVE_DIR="${SOURCE_DIR}/active"

  # Start proxy in background (it will retry connections to gateway)
  node /opt/moltbot/setup-proxy.js &
  proxy_pid=$!
