# Moltbot HA Add-on - Configuration Guide

Complete reference for all configuration options in the Moltbot Home Assistant Add-on.

---

## Configuration Overview

The Moltbot add-on can be configured in three ways:

1. **Add-on Configuration Tab** (Home Assistant UI) - For add-on-specific settings
2. **Moltbot Config File** (`/config/moltbot/data/moltbot.json`) - For Moltbot-specific settings
...
- **Use Case:** Set up ChatGPT/Codex OAuth or set API keys without needing to enter the container
...
- **Default:** `2222`
- **Range:** `1024-65535`
- **Description:** Port for SSH access (only relevant if SSH is enabled)
...
- **Type:** `string`
- **Default:** `""`
- **Description:** Public SSH keys (empty = SSH disabled)
- **Security:** **Key-based only** (no passwords)
...
- **Default:** `stable`
- **Description:** How the add-on handles Moltbot updates
...
- **Type:** `string`
- **Default:** `""`
- **Description:** Pin to a specific Moltbot version (overrides `update_mode`)
...
These settings are **not** configured as add-on options. Configure them in the **Moltbot config** (`/config/moltbot/data/moltbot.json`) or via the Web UI:
...
## Moltbot Configuration File

The main Moltbot configuration is stored in:
```
/config/moltbot/data/moltbot.json
```
...
**Note:** SSH is optional and only runs when `ssh_authorized_keys` is set in the add-on configuration.

#### Via File Editor Add-on
1. Install **File Editor** add-on
2. Navigate to `/config/moltbot/data/moltbot.json`
3. Edit and save
...
```json
{
  ...
  "workspace": {
    "path": "/config/moltbot/data/workspace",
    "max_size_mb": 500
  }
}
```
...
```json
{
  "backup": "hot",
  "backup_exclude": [
    "source/moltbot-src/.git",
    "source/moltbot-src/node_modules",
    ".npm",
    ".cache"
  ]
}
...
**What's included:**
- ✅ User data (`/config/moltbot/data/`)
- ✅ Cached versions (`/config/moltbot/cache/`)
- ✅ Version metadata (`/config/moltbot/.meta/`)
...
1. **SSH (optional, Key-based):**
   - Set `ssh_authorized_keys` only if you really need SSH
   - PasswordAuthentication is disabled (no passwords)

2. **API Keys:**
   - Store in `moltbot.json` (not in add-on config)
   - Never commit to Git
   - Rotate periodically
...
# Cached versions
ls -lh /config/moltbot/cache/
```

**Note:** The SSH commands only work if SSH was enabled in the add-on configuration via `ssh_authorized_keys`.
...
ssh -p 2222 root@YOUR-HA-IP
cd /config/moltbot/data
mv moltbot.json moltbot.json.backup
...
**Configuration complete! Your Moltbot add-on is ready to use.**
